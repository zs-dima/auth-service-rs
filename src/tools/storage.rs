...
impl S3Config {
    ...
    pub fn from_url(url: &str, access_key_id: String, secret_access_key: String) -> Result<Self, AppError> {
        ...
        if bucket.is_empty() {
            return Err(AppError::InvalidArgument("S3 URL must contain bucket name".to_string()));
        }
...
/// S3 storage client for avatar operations
#[derive(Clone)]
        ...
        match self.client.head_object().bucket(&self.bucket).key(&key).send().await {
            ...
            Err(e) => {
                ...
                if service_err.is_not_found() {
                    ...
                    Err(AppError::Internal(format!("Failed to check avatar: {service_err}")))
                }
            }
        }
    }
